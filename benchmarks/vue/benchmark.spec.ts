...
const clientKindList = ['convex-vue', 'convex-pulse'] as const

/* eslint-disable no-await-in-loop -- Browser benchmark samples must run serially to avoid workload interference. */
test('benchmarks convex-vue and convex-pulse Vue query and mutation rendering', async ({
  page
}) => {
  const iterationCount = readPositiveInteger('BENCHMARK_VUE_ITERATIONS', 20)
  const warmupCount = readPositiveInteger('BENCHMARK_VUE_WARMUPS', 5)
  const payload = 'x'.repeat(
    readPositiveInteger('BENCHMARK_PAYLOAD_BYTES', 128)
  )
  const resultList: VueBenchmarkResult[] = []
...
function renderResults(resultList: readonly VueBenchmarkResult[]) {
  const lineList = [
    'Vue render latency (browser ms)',
...
type VueBenchmarkResult = Readonly<{
